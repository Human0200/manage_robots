...
2. Извлекает результаты задачи через `tasks.task.result.list`
3. Если есть файл в результате - записывает его в указанное поле
4. Если файла нет, но есть текстовый результат - записывает текст
5. Если результатов нет - записывает описание задачи
6. Обновляет соответствующую сущность через CRM API
